      ...
      else
        {
          ...
          if (diff > m_gamma && (tcb->m_cWnd < tcb->m_ssThresh))
            ...
            {     
              ...
              if ((m_beta > diff) && (diff > m_alpha)) {
                  if (m_minRtt.GetSeconds () < m_pRtt.GetSeconds ()) {
                     segCwnd++;
                     tcb->m_cWnd = segCwnd * tcb->m_segmentSize;
                     tcb->m_ssThresh = GetSsThresh (tcb, 0);
                     m_alpha++;
              ...
              else if (diff < m_alpha)
                {
                 ...
                 if (m_alpha > 1 && m_minRtt.GetSeconds () < m_pRtt.GetSeconds ()) { 
                    segCwnd++;
                    tcb->m_cWnd = segCwnd * tcb->m_segmentSize;
                    NS_LOG_DEBUG ("Updated cwnd = " << tcb->m_cWnd <<
                                  " ssthresh=" << tcb->m_ssThresh);
                    }
                 else if (m_alpha > 1  && m_minRtt.GetSeconds () > m_pRtt.GetSeconds ()) { 
                    segCwnd--;
                    m_alpha--;
                    m_beta--;
                    tcb->m_cWnd = segCwnd * tcb->m_segmentSize;
